...
function NewPatientForm({ ctx, onClose }: { ctx: ToolContext; onClose: () => void }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      <div>
        <label className="block text-xs font-semibold text-stone-600 mb-1">{t.patients.name}</label>
        <input
          autoFocus
          value={name}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-semibold text-stone-600 mb-1">{t.patients.age}</label>
          <input
            type="number"
            min={0}
            max={120}
        ...
        <div>
          <label className="block text-xs font-semibold text-stone-600 mb-1">{t.patients.sex}</label>
          <select
            value={sex}
            onChange={(e) => setSex(e.target.value as Sex)}
            className="w-full rounded-lg border border-stone-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 bg-white"
          >
      ...
      <div>
        <label className="block text-xs font-semibold text-stone-600 mb-1">{t.patients.langPref}</label>
        <select
          value={langPref}
