...
>(({ className, hasError, ...rest }, forwardedRef) => {
  return (
    <input
      ref={forwardedRef}
      className={cn(
        'h-12 w-full min-w-0 rounded-10 bg-bg-white-0 text-center text-sm font-medium text-text-sub-600 shadow-regular-xs outline-none ring ring-inset ring-stroke-soft-200',
        'transition duration-200 ease-out',
        // hover
        'hover:bg-bg-weak-50 hover:shadow-none hover:ring-transparent',
        // focus
        'focus:shadow-gray-shadow-2 focus:outline-none focus:hover:bg-bg-white-0 focus:ring-transparent',
        // selection
        'selection:bg-none',
        // disabled
        'disabled:bg-bg-weak-50 disabled:text-text-disabled-300 disabled:shadow-none disabled:ring-transparent',
        {
          'ring-error-base hover:ring-error-base focus:ring-error-base focus:shadow-button-error-focus focus:hover:bg-bg-white-0':
            hasError,
        },
        className,
      )}
